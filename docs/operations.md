...
Build before switching: `nix build path:.#nixosConfigurations.homelab.config.system.build.toplevel`.
Keep an SSH session open, then use `nh os switch path:.`. This repository update
...
SSH is intentionally not open on the LAN or public interfaces. Adapt
[`tailscale-policy.example.hujson`](tailscale-policy.example.hujson) and apply it
in the Tailscale admin console, then connect from a permitted device with
`ssh <user>@<host>`. Choose `autogroup:self` for a user-owned node or a dedicated
tag for a shared server. Tailscale SSH verifies the
...
For clients that cannot complete an interactive browser check, use an explicit
`action: "accept"` rule only for narrowly scoped identities and target users.
Connect to the MagicDNS hostname or 100.x address on port 22 with the configured
local username and no password/private key.
...
For example, Immich uses `https://<host>.<tailnet>.ts.net:8443/`, with no `/immich`
...
`sudo smbpasswd -a <user>` to enroll the configured account. New files belong to
...
Vaultwarden registration remains closed by default. A fresh installation needs
...
The backup runs daily around 03:00 in the host's configured timezone and writes
to `homelab.backup.repository`. This is distinct from `/persist/secrets`, which
contains credentials. Unless the configured destination is on independent storage, this is

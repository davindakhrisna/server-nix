{
  ...
  hasLocalInventory = builtins.pathExists ./_local.nix;
  host =
    if hasLocalInventory
    then import ./_local.nix
    else null;
in {
  # Never evaluate a real host with generic identity defaults. In a clean
  # clone only the template configuration exists until _local.nix is created.
  flake.nixosConfigurations = lib.optionalAttrs hasLocalInventory {
